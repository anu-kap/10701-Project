...
class BasicStrategy:
    ...
    SOFT_DOUBLE = {
        13: [5, 6], 14: [5, 6], 15: [4, 5, 6],
        16: [4, 5, 6], 17: [3, 4, 5, 6], 18: [3, 4, 5, 6],
    }
    HARD_HIT = {
        ...
        12: [2, 3, 7, 8, 9, 10, 1],
        13: [7, 8, 9, 10, 1], 14: [7, 8, 9, 10, 1],
        15: [7, 8, 9, 10, 1], 16: [7, 8, 9, 10, 1],
    }
    SOFT_HIT = {
...
def persona_action(state, theta):
    ...
    if is_first_action and total in range(9, 12) and theta > 0.5:
        double_prob = (theta - 0.5) * 0.6
        if np.random.random() < double_prob:
            return 2
...
class ThetaQLearningAgent:
    ...
    def _best_action(self, state, persona_id):
        q_vals = [self.Q[(state, persona_id, a)] for a in range(NUM_ACTIONS)]
        if not state[3]:
            q_vals[2] = -float('inf')
    ...
    def update(self, state, persona_id, action, reward, next_state, done):
        ...
        if done:
            ...
            best_next = max(
                self.Q[(next_state, persona_id, a)] for a in range(NUM_ACTIONS)
            )
...
def evaluate_random(num_episodes=100_000):
    env = BlackjackEnv(num_decks=6)
    return _eval(env, lambda s: np.random.randint(NUM_ACTIONS) if s[3]
                                else np.random.randint(2), num_episodes)
...
def plot_learning_curve(win_log, save_path="learning_curve_tabular.png"):
    episodes, rates = zip(*win_log)
    plt.figure(figsize=(8, 4))
    plt.plot(episodes, rates)
    plt.xlabel("Episode")
    plt.ylabel("Win rate")
    plt.title("Tabular Q-learning — win rate over training")
    plt.tight_layout()
...
def plot_policy_heatmap(agent, save_path="policy_heatmap_tabular.png"):
    bs      = BasicStrategy()
    ...
    for pid in range(NUM_PERSONAS):
        ...
        for i, total in enumerate(totals):
            for j, upcard in enumerate(upcards):
                grid[i, j] = agent.policy((total, upcard, False, True), pid)
        axes[pid].imshow(grid, cmap=cmap, vmin=0, vmax=2, aspect="auto")
        axes[pid].set_title(f"{PERSONAS[pid]['name']}\n(θ={PERSONAS[pid]['theta']})", fontsize=9)
    ...
    fig.colorbar(im, ax=axes.tolist(), ticks=[0, 1, 2], label="0=Stand  1=Hit  2=Double")
    plt.suptitle("Tabular Q-learning policy per persona vs basic strategy", fontsize=11, y=1.02)
    plt.tight_layout()
...
def plot_theta_vs_house_edge(results_by_persona, results_bs,
                              save_path="theta_vs_edge_tabular.png"):
    ...
    plt.plot(thetas, edges, marker='o', label="Tabular Q-agent")
    ...
    plt.title("House edge vs player aggression — Tabular Q-learning")
...
if __name__ == "__main__":
    print("=" * 55)
    print("Training tabular Q-learning agent (with double down)...")
    print("=" * 55)
    ...
    for pid in range(NUM_PERSONAS):
        ...
        print(f"{label:<30} {res['win_rate']:>6.3f} {res['loss_rate']:>6.3f} "
              f"{res['push_rate']:>6.3f}  {res['expected_return']:>+10.4f}  "
